...
namespace MG {
	...
	namespace algorithm {
		...
		// Generic swap fun
		template <typename T>
		static constexpr void swap(T& Var1, T& Var2)
		{
	...
	namespace utility {
		...
		namespace str_util {
			...
			static char* StrCopy(char* des, const char* Src)
			{
				...
				while (pos <= len) des[pos] = Src[pos++];
				return des;
			}
			static char* StrCopy(char* des, const char* Src, uint64_t n)
			{
				uint64_t pos = 0;
				while (pos <= n) des[pos] = Src[pos++];
			...
			static char* StrCopy(char* des, const char* Src, uint64_t Src_pos, uint64_t len)
			{
				...
				while (Src_pos <= end) des[pos++] = Src[Src_pos++];
			...
			static uint64_t find_noCs(const std::string& Str, const char* S, uint64_t pos = 0) noexcept
			{
				...
				if (S_len == 1) return find_noCs(Str, S[0], pos);
				...
				for (uint64_t Str_index(pos); Str_index < Str_len; Str_index++)
				{
					if (std::tolower(Str[Str_index]) == std::tolower(S[0]))
					{
						...
						for (uint64_t S_index(1); S_index < S_len; S_index++)
						{
							...
							if (std::tolower(S[S_index]) != std::tolower(Str[Str_index])) break;
			...
			{
				for (char& C : _Str) C = char((int)C + (Enc_Key >> 1));
				return _Str;
			}
			static std::string _Decrypt_(std::string _Str, int Dec_Key)
			{
				for (char& C : _Str) C = char((int)C - (Dec_Key >> 1));
				return _Str;
			}
		...
		namespace arr_util {
			...
			//-----------------------------------------------------------------------------------
			template <typename T, uint64_t _size> constexpr uint64_t arr_size(const T(&)[_size]) noexcept { return _size; }
			...
			// (comp) function pointer take -> Max() or Min() functions
			// (comp = min) -> Progressive
			// (comp = max) -> descending
			...
			template <typename T, uint64_t _size>
			constexpr int32_t sum(T(&arr)[_size])
			{
				int32_t sum(0);
				for (uint64_t i(0); i < _size; i++)
					sum += arr[i];
			...
			// linear search (ls)
			template <typename T, uint64_t _size>
			constexpr uint64_t ls_find(T(&arr)[_size], const T& _elem)
			{
				for (uint64_t i(0); i < _size; i++)
					if (_elem == arr[i]) return i;
				return UINT64_MAX;
			}
			...
			constexpr uint64_t ls_sfind(std::string(&arr)[_size], const std::string& _elem)
			{
				for (uint64_t i(0); i < _size; i++)
					if (_elem == arr[i]) return i;
				return UINT64_MAX;
			...
			constexpr inline uint64_t bs_find(Ty(&arr)[n], const Ty& _elem, uint64_t first = 0, uint64_t last = n - 1, bool not_Sorted = true)
			{
				if (not_Sorted) {
					sort(arr); // array must be sorted for binary search
				}
				// first & last & mid indexes of arr
				uint64_t mid(0);
				while (first <= last)
				{
					mid = (first + last) / 2;
					if (_elem == arr[mid]) return mid;
					if (_elem > arr[mid]) first = mid + 1;
					else last = mid - 1;
				}
				return UINT64_MAX;
			}
