...
namespace MG {
	...
	namespace utility {
		...
		namespace arr_util {
			...
			// linear search (ls)
			template <typename T, uint64_t _size>
			constexpr uint64_t linear_search(T(&arr)[_size], const T& _elem)
			{
				for (uint64_t i(0); i < _size; i++)
			...
			constexpr inline uint64_t binary_search(Ty(&arr)[n], const Ty& _elem, bool not_Sorted = false, uint64_t first = 0, uint64_t last = n - 1)
			{
