...
void Space(short NumberOfSpaces)
{
...
void UnderScore(short NubmerOfUnderScores)
{
...
enum en_Choices { Paper = 1, Stone = 2, Scissors = 3 };
enum en_RoundWinner { Draw = 1, PlayerWins = 2, ComputerWins = 3 };

struct st_GameResults
{
	short PlayerWinTimes, ComputerWinTimes, DrawTimes, GameRounds;
	string FinalWinner;
};

en_Choices Choise(short Choise)
{
	switch (Choise)
	{
	case 1:
		return en_Choices::Paper;
	case 2:
		return en_Choices::Stone;
	case 3:
		return en_Choices::Scissors;
	}
}

string Get_Choice(en_Choices Choice)
{
	switch (Choice)
	{
	case Paper:
...
en_RoundWinner Get_RoundResult(en_Choices PlayerChoice, en_Choices ComputerChoice)
{
	switch (PlayerChoice)
	{
	case Paper:
		if (ComputerChoice == Paper)
			return en_RoundWinner::Draw;
		else if (ComputerChoice == Stone)
			return en_RoundWinner::PlayerWins;
		else if (ComputerChoice == Scissors)
			return en_RoundWinner::ComputerWins;
	case Stone:
		if (ComputerChoice == Paper)
			return en_RoundWinner::ComputerWins;
		else if (ComputerChoice == Stone)
...
void Print_RoundResult(string PlayerChoice, string ComputerChoice, en_RoundWinner RoundResult, short& PlayerWinTimes, short& ComputerWinTimes, short& DrawTimes)
{
	cout << "- Player Chioce   : { " << PlayerChoice << " }" << endl;
	cout << "- Computer Chioce : { " << ComputerChoice << " }" << endl;
	cout << "- Round Winner    : { ";
...
void Round(short RoundNumber, short& PlayerWinTimes, short& ComputerWinTimes, short& DrawTimes)
{
	cout << "- Round[" << RoundNumber << "] Starts:" << endl;
	star(19);
	NewLine(2);

	short PlayerChoice = Read_Positive_Int_NumberInRange_Message("- What Is Your Choice? { [1]Paper [2]Stone [3]Scissors } / ", 1, 3);
	short ComputerChoice = Generate_Random_Number_InRange(1, 3);

	en_RoundWinner RoundResult = Get_RoundResult(Choise(PlayerChoice), Choise(ComputerChoice));
	...
	Print_RoundResult(Get_Choice(Choise(PlayerChoice)), Get_Choice(Choise(ComputerChoice)), RoundResult, PlayerWinTimes, ComputerWinTimes, DrawTimes);
...
string Get_FinalWinner(short PlayerWinTimes, short ComputerWinTimes)
{
	if (PlayerWinTimes == ComputerWinTimes)
	{
		system("color 6F");
		return "No Winner _ Draw _";
	}
...
void ResetGame(st_GameResults& GameResults)
{
	system("cls");
	system("color 0F");
...
void Play_Paper_Stone_Scissors_Game()
{
	...
	do
	{
		...
		GameResults.GameRounds = Read_Positive_Int_NumberInRange_Message("- How Many Rounds You Want To Play? / ", 1, 10);

		for (short i = 1; i <= GameResults.GameRounds; i++)
		{

			Round(i, GameResults.PlayerWinTimes, GameResults.ComputerWinTimes, GameResults.DrawTimes);

			NewLine(3);

		}

		GameResults.FinalWinner += Get_FinalWinner(GameResults.PlayerWinTimes, GameResults.ComputerWinTimes);
...
int main()
{
	...
	Play_Paper_Stone_Scissors_Game();
