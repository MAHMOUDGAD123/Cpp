...
void Fill_Array_With_Random_Numbers_Length(short Array[100], short& Arr_Length)
{
	Arr_Length = Read_Positive_LLInt_NumberInRange("Please, Enter The Array Length?", 1, 100);
...
void Fill_Array_With_Shuffled_Numbers_From1toN_WithoutRepeating(short Array[], short Arr_Length)
{
	short X = 0;
	...
	for (short i = 0; i < Arr_Length; i++)
	{
		do
		{
			X = _Generate_Random_Number_InRange(1, Arr_Length);

		} while (_Get_Number_Frequency_InArray(Array, X, Arr_Length) != 0);
...
void Shuffle_Array_MG(short Array[], short Arr_Length)
{
	...
	for (short i = 0; i < Arr_Length; i++)
	{
		N1 = _Generate_Random_Number_InRange(0, Arr_Length - 1);
		N2 = _Generate_Random_Number_InRange(0, Arr_Length - 1);

		_Swap2Numbers(Array[N1], Array[N2]);
	}
}
...
void Shuffle_Array_Hadhoud(short Array[], short Arr_Length)
{
	for (short i = 0; i < Arr_Length; i++)
	{
		_Swap2Numbers(Array[_Generate_Random_Number_InRange(0, Arr_Length - 1)], Array[_Generate_Random_Number_InRange(0, Arr_Length - 1)]);
	}
}
...
int main()
{
	...
	short Array[100], Arr_Length;

	Arr_Length = Read_Positive_LLInt_Number("Please, Enter The Number Of Elements?");

	Fill_Array_With_Orderd_Numbers_From1toN(Array, Arr_Length); // Arr_Length = N

	cout << "-> Array Befoe Shuffling : { ";
	_Print_Array(Array, Arr_Length);
	cout << "}" << endl << endl;

	// MG
	/*Array_ZeroFill(Array, Arr_Length);
	Fill_Array_With_Shuffled_Numbers_From1toN_WithoutRepeating(Array, Arr_Length);*/

	// Hadhoud

	//Shuffle_Array_MG(Array, Arr_Length);

	Shuffle_Array_Hadhoud(Array, Arr_Length);
