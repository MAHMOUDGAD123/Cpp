...
void Print_IntArray(short Array[], short Arr_Length)
{
...
{
	Arr_Length = Read_Positive_LLInt_Number("Please, Enter The Array Length?");

	short N = Read_Positive_LLInt_Number("Enter The Range Of The Array Element?");

	for (short i = 0; i < Arr_Length; i++)
	{
		Array[i] = Generate_Random_Number_InRange(1, N);
	}

	cout << endl;
}

void Fill_Array_With_Random_From1toN_Numbers(short Array[], short Arr_Length)
{
	short N = Read_Positive_LLInt_Number("Enter The Range Of The Array Element?");
...
void Fill_Array_With_Random_From1toN_Numbers_NoRepeating(short Array[], short Arr_Length)
{
	short X = 0;
	short N = Read_Positive_LLInt_Number("Enter The Range Of The Array Element? (No Repeating)");

	for (short i = 0; i < Arr_Length; i++)
	{
		do
		{
			X = Generate_Random_Number_InRange(1, N);
...
// MG Solution
void _Print_IntArray(short Array[], short Arr_Length)
{
	for (short i = 0; i < Arr_Length; i++)
	{
		cout << Array[i] << " ";
...
// No Repeating
void _Fill_Array_With_Random_Numbers_From1toN_NoRepeating(short Array[], short Arr_Length)
{
	// Be careful if (Arr_Length > N) the function won't work because -> repeating is not allowed in this function

	short X = 0;
	short N = Read_Positive_LLInt_Number("Enter The Range Of Array Element? (No Repeating)");
...
short GetNumberIndex_InArray_NR(short Array[], short Arr_Length, short NumberToCheck)
{
...
void Print_SearchResults_NR(short Array[], short Arr_Length, short NumberToCheck)
{
	short Result = GetNumberIndex_InArray_NR(Array, Arr_Length, NumberToCheck);
	cout << "*********************************************" << endl << endl;
	cout << "# The Number You Are Searching For Is : (" << NumberToCheck << ")" << endl << endl;

	if (Result >= 0)
	{
		cout << "-> We Found It At Index : {" << Result << "}" << endl;
		cout << "-> The Number Order in Array is : {" << Result + 1 << "}" << endl;
	}
	else
	{
		cout << "-> The Number you Are Searching for is not found :-(" << endl;
	}
...
void _Fill_Array_With_Random_From1toN_Numbers(short Array[], short Arr_Length)
{
	short N = Read_Positive_LLInt_Number("Enter The Range Of The Array Element?");
...
short _Get_Number_Frequency_InArray(short Array[], short NumberToCheck, short Arr_Length)
{
	// Return the number of repeating times of a number in the array
...
string Get_NumberIndexes_R(short Array[], short Arr_Length, short NumberToCheck)
...
void Print_SearchResults_R(short Array[], short Arr_Length, short NumberToCheck)
{
	string Indexes = Get_NumberIndexes_R(Array, Arr_Length, NumberToCheck);
	short RepeatingTimes = Get_Number_Frequency_InArray(Array, NumberToCheck, Arr_Length);
	...
	if (RepeatingTimes > 0)
	{
		cout << "We Found Your Number " << RepeatingTimes << " Time(s) At The Next Indexes : " << Indexes << endl;
	}
	else
	{
		cout << "-> The Number you Are Searching for is not found :-(" << endl;
...
int main()
{
	...
	short Array[100], Arr_Length = 0;
	Arr_Length = Read_Positive_LLInt_NumberInRange("How Many Number Do You Want In The Array?", 1, 100);

	// Repeating Not Allowed
	/*_Fill_Array_With_Random_Numbers_From1toN_NoRepeating(Array, Arr_Length);

	cout << "Array Elements: { ";
	_Print_IntArray(Array, Arr_Length);
	cout << "}" << endl << endl;
	
	short NumberToCheck = Read_Positive_LLInt_Number("What Number Do You Want To Search For?");

	Print_SearchResults_NR(Array, Arr_Length, NumberToCheck);*/



	// Repeating Allowed
	_Fill_Array_With_Random_From1toN_Numbers(Array, Arr_Length);
	...
	Print_SearchResults_R(Array, Arr_Length, NumberToCheck);
