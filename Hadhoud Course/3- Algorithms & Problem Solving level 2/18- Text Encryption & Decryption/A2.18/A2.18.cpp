...
char Encrypt_Letters(char C , short Enc_Key)
{
	if (C != ' ')
	{
		short C_to_Int = int(C);
		C_to_Int += Enc_Key;
		C = char(C_to_Int);
	}
	return C;
...
char Decrypt_Letters(char C, short Dec_Key)
{
	if (C != ' ')
	{
		short C_to_Int = int(C);
...
string Encryption(string Text, short Enc_Key)
{
	for (short i = 0; i < Text.length(); i++)
	{
		Text[i] = Encrypt_Letters(Text[i], Enc_Key);
	}

	return Text;
}

string Decryption(string Text, short Dec_Key)
{
	for (short i = 0; i < Text.length(); i++)
	{
		Text[i] = Decrypt_Letters(Text[i], Dec_Key);
	}
...
string Encrypt_Text(string Text, short Enc_Key)
{
	for (short i = 0; i < Text.length(); i++)
...
string Decrypt_Text(string Text, short Dec_Key)
{
	for (short i = 0; i < Text.length(); i++)
		Text[i] = char((int)Text[i] - Dec_Key);
...
int main()
{
	string Text = Read_Text("Please, Enter A Text To Encrypt it?");
	const short Enc_Key = 3;
	
	//// MG Solution
	cout << " => Text Before Encryption : " << Text << endl;
	cout << " => Text After Encryption : " << Encryption(Text, Enc_Key) << endl;
	cout << " => Text After Decryption : " << Decryption(Encryption(Text, Enc_Key), Enc_Key) << endl;
	...
	// Hadhoud Solution
	string TextAfterEncryption = Encrypt_Text(Text, Enc_Key);
	string TextAfterDncryption = Decrypt_Text(TextAfterEncryption, Enc_Key);

	cout << " => Text Before Encryption : " << Text << endl;
	cout << " => Text After Encryption : " << TextAfterEncryption << endl;
	cout << " => Text After Decryption : " << TextAfterDncryption << endl;
