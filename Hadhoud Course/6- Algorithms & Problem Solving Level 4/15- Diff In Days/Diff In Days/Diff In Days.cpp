...
unsigned short NumberOfDaysFromTheBeginningOfTheYear(sDate Date)
{
...
sDate DateAddDays(sDate Date, size_t DaysToAdd)
{
	...
	size_t RemainingDays = DaysToAdd + NumberOfDaysFromTheBeginningOfTheYear(Date);
...
size_t GetDifferenceInDays(sDate StartDate, sDate EndDate, bool IncludeEndDay = false)
{
	...
	unsigned short StartDateDays = NumberOfDaysFromTheBeginningOfTheYear(StartDate);
	unsigned short EndDateDays = NumberOfDaysFromTheBeginningOfTheYear(EndDate);

	while (CurrentYear < EndDate.Year)
		DiffDays += NumberOfDaysInAYear(CurrentYear++);

	DiffDays += (size_t)(EndDateDays - StartDateDays);

	return IncludeEndDay ? ++DiffDays : DiffDays;
}

size_t _GetDifferenceInDays(sDate StartDate, sDate EndDate, bool IncludeEndDay = false)
{
	size_t DiffDays = 0;
	...
	// Number of days till the end of the year for 'StartDate'
	unsigned short NumberOfDaysLeftInYear = NumberOfDaysTillTheEndOfTheYear(StartDate);

	// Add (NumberOfDaysLeftInYear + 1) to both dates to make 'StartDate' ---> { 1/1/year }
	StartDate = DateAddDays(StartDate, (size_t)(NumberOfDaysLeftInYear + 1));
	EndDate = DateAddDays(EndDate, (size_t)(NumberOfDaysLeftInYear + 1));

	// Adding difference between years in days
	for (unsigned short year = StartDate.Year; year < EndDate.Year; year++)
		DiffDays += NumberOfDaysInAYear(year);

	// Adding the Days of the last year (EndDate.Year)
	DiffDays += NumberOfDaysFromTheBeginningOfTheYear(EndDate);

	// End_Day is already included
	return (IncludeEndDay) ? DiffDays : --DiffDays;
}

size_t __GetDifferenceInDays(sDate StartDate, sDate EndDate, bool IncludeEndDay = false)
{
	size_t Days = 0;
...
int main(void)
{
	...
	for (;;)
	{
		...
		printf("\n-> Date1 Is:  %2hu/%2hu/%llu", Date1.Day, Date1.Month, Date1.Year);
		printf("\n-> Date2 Is:  %2hu/%2hu/%llu\n\n", Date2.Day, Date2.Month, Date2.Year);


		printf("-> Difference Is:                          %llu Day(s)\n", GetDifferenceInDays(Date1, Date2));
		printf("-> Difference (Including The End Day) Is:  %llu Day(s)\n\n", GetDifferenceInDays(Date1, Date2, true));

		printf("-> Difference Is:                          %llu Day(s)\n", _GetDifferenceInDays(Date1, Date2));
		printf("-> Difference (Including The End Day) Is:  %llu Day(s)\n\n", _GetDifferenceInDays(Date1, Date2, true));
