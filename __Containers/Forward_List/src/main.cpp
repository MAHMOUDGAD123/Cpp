...
#include "../Header Files\forward_list.h"
...
constexpr static void Print(const MG::forward_list<_Ty> &Flist)
{
	std::cout << "\n\n";
	if (Flist.empty())
	{
...
auto greater = [&]<typename _Ty>(const _Ty &_lhs, const _Ty &_rhs)
{ return _lhs > _rhs; };
auto less = [&]<typename _Ty>(const _Ty &_lhs, const _Ty &_rhs)
{ return _lhs < _rhs; };
